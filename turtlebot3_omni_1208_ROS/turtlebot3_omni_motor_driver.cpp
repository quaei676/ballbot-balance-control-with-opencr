...
bool Turtlebot3MotorDriver::setTorque(uint8_t id, bool onoff)
{
  ...
  {
    packetHandler_->getTxRxResult(dxl_error);
  }
}

bool Turtlebot3MotorDriver::readEncoder(int32_t &first_wheel_value, int32_t &second_wheel_value, int32_t &third_wheel_value)
{
  ...
  if (dxl_getdata_result != true)
    return false;
  // Get data
  first_wheel_value  = groupSyncReadEncoder_->getData(first_wheel_id_,  ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  second_wheel_value = groupSyncReadEncoder_->getData(second_wheel_id_, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
  third_wheel_value  = groupSyncReadEncoder_->getData(third_wheel_id_,  ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
